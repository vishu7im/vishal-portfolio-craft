...
- [x] **Phase 11 — Visual art-direction & post-FX pass** ✅ _(committed directly to `main` — the frame-level "look" pass)_
  - [x] **Colour grade** — a CSS `filter: saturate(1.16) contrast(1.05) brightness(1.01)` on the game canvas (`Game.tsx`), so the whole composited frame reads richer and punchier instead of pastel-flat — globally, without touching every system's colour constants
  - [x] **Cinematic vignette** — new `WorldVignette` DOM overlay (`HudExtras`): a radial-gradient that darkens the corners for depth/focus and **deepens after dark**, its opacity driven by a new `frame.nightness` (pushed by `DayNightSystem`) via rAF. Sits below the HUD (`z-[5]`) so only the world dims, never the readouts. Ties the look to the Phase 8 day/night — day gets subtle depth, night gets a heavy moody frame
  - [x] **Deliberately NOT Phaser WebGL post-FX** — I first built a `PostFxSystem` using Phaser's camera `postFX` (bloom + vignette + ColorMatrix) and **backed it out**: (a) Phaser's Bloom has no brightness threshold, so on our bright-paper palette it lifted the whole frame and blew the night out to cream; (b) the camera post-FX framebuffer **breaks the day/night MULTIPLY overlay** and the vignette shader rendered inverted/bright-cornered under software-WebGL — a look I couldn't verify as correct on real hardware. The CSS grade + vignette are **guaranteed to render identically on every device**, need no WebGL, and never fight the in-canvas tint. Net: a real frame-level upgrade with zero rendering risk
  - [x] **Deferred:** a display-typeface pass and per-element HUD easing (the roadmap's softer Phase 11 items) — the panel already fades up (Phase 9) and HUD chips transition; a characterful in-world font is left for a later polish pass to avoid a webfont/CSP dependency now
  - [x] **Verified in a headless browser** (Playwright, before/after screenshots at day/dusk/night): the canvas carries the `saturate/contrast` grade; the vignette overlay is present and its opacity climbs **0.40 (day) → 0.55 (dusk) → 0.86 (night)** with `frame.nightness`; day reads with subtle richer-green depth, night is a deep moody indigo (no blow-out, no inverted corners); the HUD stays fully legible above the vignette; the car drives; zero console errors. `tsc`/`build`/lint ✓
